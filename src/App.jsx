...
const SHEET_WEBAPP_URL=import.meta.env.VITE_GOOGLE_SCRIPT_URL||"";const APP_NAME="Gülleverschlauchung";const ANLAGEN=["Anlage1","Anlage2"];
function parseDecimal(v){if(v===null||v===undefined)return NaN;const c=String(v).trim().replace(/\s/g,"").replace(",",".");return c===""?NaN:Number(c)}
function formatNumber(v){if(v===""||Number.isNaN(v)||v===null||v===undefined)return"—";return Number(v).toLocaleString("de-AT",{minimumFractionDigits:1,maximumFractionDigits:2})}
function pad2(v){return String(v).padStart(2,"0")}function secondsToClock(t){const s=Math.max(0,Math.floor(t||0)),h=Math.floor(s/3600),m=Math.floor((s%3600)/60),sec=s%60;return`${pad2(h)}:${pad2(m)}:${pad2(sec)}`}
...
async function addListItem(type){const value=(type==="driver"?newDriver:newCustomer).trim();if(!value){alert("Bitte Namen eingeben.");return}setLoading(true);try{await apiPost({action:"addListItem",type,value});if(type==="driver"){setDrivers([...new Set([...drivers,value])].sort((a,b)=>a.localeCompare(b,"de")));setNewDriver("")}else{setCustomers([...new Set([...customers,value])].sort((a,b)=>a.localeCompare(b,"de")));setNewCustomer("")}setMessage("Eintrag wurde hinzugefügt.")}catch{setMessage("Eintrag konnte nicht hinzugefügt werden.")}finally{setLoading(false)}}
